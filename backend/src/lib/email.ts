...
export async function sendWrenMagicLinkEmail(params: {
  to: string;
  link: string;
  purpose: "login" | "signup";
}) {
  ...
  const subject =
    params.purpose === "login"
      ? "Tu enlace de acceso a Numination"
      : "Tu enlace para crear tu cuenta en Numination";

  const html = `
    <div style="font-family:Inter,Arial,sans-serif;background:#0b0d10;color:#f3f5f7;padding:32px;border-radius:20px;max-width:560px;margin:0 auto">
      <h1 style="margin:0 0 16px 0;font-size:28px;letter-spacing:2px">NUMINATION</h1>
      <p style="margin:0 0 22px 0;color:#9ca3af;font-size:15px;line-height:1.6">
        Toca el botón para iniciar sesión. Este enlace solo funciona en tu teléfono, donde tengas la app instalada.
      </p>
      <a href="${params.link}" style="display:inline-block;background:#6B54F6;color:#ffffff;text-decoration:none;border-radius:14px;padding:16px 28px;font-size:16px;font-weight:700">
        Abrir Numination
      </a>
      <p style="margin:22px 0 0 0;color:#9ca3af;font-size:13px;line-height:1.6">
        Este enlace expira en 15 minutos y solo se puede usar una vez. Si no pediste este acceso, puedes ignorar este correo.
      </p>
    </div>
  `;
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      subject,
      html,
      text: `Numination\n\nAbre este enlace en tu teléfono para iniciar sesión:\n${params.link}\n\nEste enlace expira en 15 minutos.`,
    }),
  });

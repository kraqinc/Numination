...
async function loadJwks(
  ...
  if (!keys.length) {
    throw new Error("Supabase JWKS contains no keys");
  }
...
function verifyAsymmetricJwt(
  ...
  const header = decodeJsonPart<{
    alg?: string;
    kid?: string;
  }>(encodedHeader);

  const payload = decodeJsonPart<JwtPayload>(
    encodedPayload,
  );

  if (header.alg !== "ES256" && header.alg !== "RS256") {
    throw new Error(
      `Unsupported Supabase JWT algorithm: ${
        header.alg ?? "unknown"
      }`,
    );
  }
  ...
  if (!candidates.length) {
    throw new Error("No matching Supabase signing key");
  }
  ...
  for (const jwk of candidates) {
    try {
      ...
      // Try the next available signing key.
    }
  }

  if (!valid) {
    throw new Error("Invalid Supabase JWT signature");
  }
  ...
  if (
    ...
    throw new Error(
      "Supabase JWT is not active yet",
    );
  }

  if (!payload.sub) {
    throw new Error(
      "Supabase JWT has no subject",
    );
  }
...
export async function verifySupabaseAccessToken(
  ...
  try {
    ...
    try {
      ...
      const legacy =
        await verifyLegacyTokenWithAuthServer(
          token,
          supabaseUrl,
        );
    ...
    const profile =
      await findProfileWithRetry(sub);
